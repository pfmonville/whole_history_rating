...
This guide documents the complete `whole-history-rating` API, including
advanced uncertainty, draw, contextual-advantage, convergence, and
configuration workflows. For the short introduction and installation path, see
the [project README](https://github.com/pfmonville/whole_history_rating#readme).
...
with the same metric. Everything below is reproducible from the
[benchmark suite](https://github.com/pfmonville/whole_history_rating/tree/master/benchmarks);
the full write-up, method and caveats are in the
...
> **About the numbers in this guide.** Every output shown below is the real,
